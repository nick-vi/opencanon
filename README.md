...
- `search` searches symbols, conventions, checks, docs, and exact indexed context. `ask` queries the local Project Context index with cited evidence.
...
OpenCanon uses one lightweight global service registered in `~/.opencanon/service.json`. The service discovers OpenCanon projects from the current working directory, lazily starts an isolated per-project runtime on demand, and lists registered project runtimes across projects. Each project runtime keeps its own `.opencanon/state.sqlite`, log, auth token, live refresh state, generated project types, and idle timeout. The registry records both a local pipe endpoint and a loopback browser URL; CLI, MCP, and runtime clients prefer the pipe endpoint, while URLs remain available for browser diagnostics.

...
use crate::contracts::{
    AppendJobEventRequest, BuildRepoGraphRequest, EmbedSemanticTextsRequest, ExtractFactsRequest,
    FactDiagnostic, GenerateTextRequest, ListEventsRequest, ListJobEventsRequest, ListJobsRequest,
    ListObservabilityRecordsRequest, OpenProjectRequest, ProjectRefreshStatus, ReadJobRequest,
    ResolvedProjectSettings, ScanAndDiffRequest, StartWatcherRequest, WatcherStartResult,
    WriteEventRequest, WriteJobRequest, WriteObservabilityRecordsRequest,
};
...
impl EngineProjectHandle {
    ...
    pub fn list_jobs_json(&self, request: String) -> napi::Result<String> {
        ...
        if let Some(job_type) = request.r#type {
            let mut statement = conn
                .prepare(
                    "select payload from jobs where type = ?1 order by updated_at desc limit ?2",
                )

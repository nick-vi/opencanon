...
export default [
  ...
  defineChange({
    ...
    intent: {
      problem: "Full project Proof omits release-owned Rust crates, private integration registries leak detached processes, derived vector files accept obsolete formats, and release version/publication checks do not cover every owned artifact.",
      outcome: "All authored release code is governed, ephemeral process owners reach a clean terminal state, old vector data rebuilds instead of loading, and one explicit audited release path moves every version together.",
      why: "A release gate is trustworthy only when its inventory, process steady state, derived data formats, dependency security, and publication trigger are complete and deterministic.",
    },
    tasks: [
      {
        id: "canon-contract",
      ...
      },
      {
        ...
        files: ["packages/runtime/test/*.test.ts", "tests/mcp.test.ts", "tests/worktree.test.ts"],
        checks: ["service-tests", "runtime-client-tests", "mcp-tests", "worktree-tests", "process-steady-state"],
        dependsOn: ["canon-contract"],
      },
      {
      ...
      },
      {
        ...
        dependsOn: ["owned-test-processes", "durable-docs-only"],
      },
    ],
    checks: [
      { id: "canon-render", kind: "command", command: "npm run opencanon -- canon render specs --dry-run" },

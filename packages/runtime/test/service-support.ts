...
export function readyFakeServiceCliSource(options: { exitOnceMarkerPath?: string; pidPathEnv?: string } = {}): string {
  return [
    ...
    "      socket.end(JSON.stringify({ protocol: 'opencanon.local.v2', id: request.id, status: 200, statusText: 'OK', body: ready }) + '\\n');",
    "    });",
    "  }).listen(pipeEndpoint);",
    "}",
    "",
...
export function readyFakeRuntimeCliSource(options: { exitOnceMarkerPath?: string; pidPathEnv?: string; startupDelayMs?: number; writeStateMarker?: boolean } = {}): string {
  return [
    ...
    "      if (!buffer.includes('\\n')) return;",
    "      const request = JSON.parse(buffer.slice(0, buffer.indexOf('\\n')));",
    "      socket.end(JSON.stringify({ protocol: 'opencanon.local.v2', id: request.id, status: 200, statusText: 'OK', body: health }) + '\\n');",
    "    });",
    "  }).listen(pipeEndpoint);",
    "}",
    "",

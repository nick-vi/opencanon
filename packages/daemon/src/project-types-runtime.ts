...
import { generateProjectTypes, relative, type ContextPaths } from "@opencanon/core";
...
export function createProjectTypesRuntime(input: {
  ...
  function startWatcher(): void {
    try {
      watcher = watch(input.rootDir, { recursive: true }, (_event, filename) => {
        if (!filename || stopped) return;
        const file = normalizeChangedPath(input.rootDir, filename.toString());
        if (!isProjectTypesGenerationInput(file, fixturePrefix(input.rootDir, input.paths()))) return;
        schedule(`Project authoring types updated after ${file} changed.`);
      });
      watcher.on("error", (error) => {
        reportGenerationMessage(`Project authoring type watcher failed: ${errorMessage(error)}`);
      });
    } catch (error) {
      reportGenerationMessage(`Project authoring type watcher failed: ${errorMessage(error)}`);
    }
...
export function isProjectTypesGenerationInput(file: string, fixturePrefix: string): boolean {
  if (file.startsWith(".agents/skills/opencanon/generated/")) return false;

...
import { UrlSearchParam, diagnostic, diagnosticCodes, json } from "./routes.ts";
...
const TreeHiddenFilePrefixes = [".agents/skills/opencanon/runtime/ui/vscode-icons/", "packages/ui/public/vscode-icons/"] as const;
...
): { ok: true; path: string } | { ok: false; error: { ok: false; diagnostics: unknown[] } } {
  const safe = safeRelativePath(input, options);
  if (safe.ok) return safe;
  return {
    ok: false,
    error: diagnostic(diagnosticCodes.invalidDaemonResponse, safe.message),
  };
}

export function validateRelativePaths(inputs: string[]): { ok: true; paths: string[] } | { ok: false; error: { ok: false; diagnostics: unknown[] } } {
  if (inputs.length === 0) {
    return {
      ok: false,
      error: diagnostic(diagnosticCodes.invalidDaemonResponse, "At least one file path is required."),
    };
  }
...
export function validateOptionalRelativePaths(inputs: string[]): { ok: true; paths: string[] } | { ok: false; error: { ok: false; diagnostics: unknown[] } } {
  const paths: string[] = [];
  for (const input of inputs) {
    const safe = validateRelativePath(input, { allowEmpty: false });
    if (!safe.ok) return safe;
...
export function validateCommitHash(input: string): { ok: true; commit: string } | { ok: false; error: { ok: false; diagnostics: unknown[] } } {
  const commit = input.trim();
  if (!commit) {
    return {
      ok: false,
      error: diagnostic(diagnosticCodes.invalidDaemonResponse, "Commit is required."),
    };
  }
  if (!/^[a-f0-9]{7,40}$/i.test(commit)) {
    return {
      ok: false,
      error: diagnostic(diagnosticCodes.invalidDaemonResponse, "Commit must be a 7 to 40 character Git hash."),
    };
  }
...
export type ProjectInventory = { ok: true; files: string[] } | { ok: false; error: { ok: false; diagnostics: unknown[] } };
...
export function buildTreeResponse(dirPath: string, projectFiles: string[], snapshot: DaemonSnapshot, options: { query: string; showDotEntries: boolean }): TreeResponse {
  ...
  const dirEntries: TreeEntry[] = [...childDirs.entries()].map(([dirKey, info]) => compactTreeDirectoryEntry(dirKey, prefix, visibleFiles, info));
  const entries = [...dirEntries.sort(compareTreeEntryName), ...directFiles.sort(compareTreeEntryName)];
...
export async function readFileResponse(cwd: string, relPath: string): Promise<Response> {
  ...
  if (!resolved.ok) return json(diagnostic(diagnosticCodes.invalidDaemonResponse, resolved.message), 400);
  const absolute = resolved.absolutePath;
  if (!existsSync(absolute)) {
    return json(diagnostic(diagnosticCodes.invalidDaemonResponse, `File not found: ${relPath}.`), 404);
  }
  const stat = statSync(absolute);
  if (!stat.isFile()) {
    return json(diagnostic(diagnosticCodes.invalidDaemonResponse, `Not a file: ${relPath}.`), 400);
  }
  if (stat.size > maxFileBytes) {
    return json(diagnostic(diagnosticCodes.invalidDaemonResponse, `File exceeds ${maxFileBytes} byte cap (${stat.size} bytes).`), 413);
  }

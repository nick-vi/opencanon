...
function renderRuntimeUpdateCheckMarkdown(check: RuntimeUpdateCheck): string {
  return [
    ...
    `Daemon schema: ${check.daemonSchema}`,
    `Engine schema: ${check.engineSchema}`,
    check.resolvedRuntimeArchiveSource ? `Runtime archive: ${check.resolvedRuntimeArchiveSource}` : "",
    check.runtimeArchiveSha256 ? `Runtime archive SHA-256: ${check.runtimeArchiveSha256}` : "",
    `Manifest: ${check.manifestSource}`,
    `Asset: ${check.resolvedAssetSource}`,
    `Runtime file: ${check.runtimePath}`,
